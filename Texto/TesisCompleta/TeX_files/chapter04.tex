\chapter{Transiciones de estabilidad (Resultados)}

\setlength{\parindent}{0cm} 

En vista de que la matriz Jacobiana $\mathcal{J}$ resulta de un reescalamiento multiplicativo por las abundancias del equilibrio $X^*$, su espectro muestra deformaciones sistemáticas con respecto al espectro de la matriz $\Lambda$. En esta sección se analizan los cambios dinámicos globales del sistema mediante barridos paramétricos análogos a los considerados en la subsección \ref{sec:transMay}, con el objetivo de caracterizar las transiciones dinámicas del sistema y explorar la posibilidad de identificar un parámetro de control crítico que permita aproximar heurísticamente el umbral de estabilidad.
\\
\\
...
\\
Asimismo, los resultados espectrales discutidos anteriormente sugieren que los estados parcialmente factibles tienden a ubicarse cerca del borde de la inestabilidad. En estos casos, una fracción de los valores propios de la Jacobiana se aproxima al origen del plano complejo, lo que implica tiempos de relajación largos y una mayor sensibilidad a perturbaciones estructurales. Desde este perspectiva, los estados parcialmente factibles pueden interpretarse como configuraciones dinámicas cercanas al régimen de estabilidad marginal.
\newpage
...
\begin{figure}[h!]
	...
	\caption{Transición dinámica del sistema de Lotka--Volterra generalizado estructuralmente simétrico en función de la probabilidad con escala logarítmica para $N=100$.}
...
En particular, las transiciones asociadas a matrices con soporte dirigido presentan una pérdida de estabilidad más abrupta conforme aumenta la probabilidad $p$, mientras que los sistemas con soporte estructuralmente simétrico exhiben una de transición más amplia. Esto indica que la estructura del soporte de $\Lambda$ influye de manera significativa en las transiciones dinámicas del sistema. En efecto, las diferencias de conectividad ---dirigida o simétrica--- modifican la forma en que el reescalamiento inducido por $X^*$ afecta la matriz efectiva $\mathcal{J}$, lo que se traduce en cambios en la distribución de valores propios relevantes para la estabilidad, y por lo tanto en la probabilidad de estabilidad observada.\\
\\
...
\\
Este comportamiento sugiere que la simetría del soporte ---que produce anisotropía y elongación del espectro hacia los ejes del plano complejo--- continúa influyendo en la probabilidad de estabilidad, aún cuando la matriz Jacobiana efectiva esté modulada por el reescalamiento multiplicativo de las abundancias del equilibrio.
\begin{figure}[h!]
	\centering
...
\begin{figure}[h!]
	\centering
	\includegraphics[scale=0.2]{../Imagenes/ComparacionIncidenciasσ}
...
\begin{figure}[h!]
	...
	\caption{Transición dinámica del sistema de Lotka--Volterra generalizado en función de la fuerza de interacción para $N=25$.}
...
\newpage
\section{Parámetro heurístico de transición dinámica}
...
\end{wrapfigure}
Como se ha mencionado anteriormente, una forma de aproximar el umbral de estabilidad del sistema consiste en analizar la estructura espectral de la matriz $\Lambda$. En  particular, sus valores propios proporcionan una escala natural que influye en la magnitud de las abundancias del equilibrio $X^*$. Cuando algún valor propio de $\Lambda$ se aproxima a cero, $\Lambda$ se vuelve cercana a singular, lo que puede inducir grandes variaciones en las abundancias del equilibrio $X^*$. En estas condiciones, el sistema tiende a situarse cerca del borde de inestabilidad, ya que la estructura de la Jacobiana puede volverse altamente sensible a pequeñas perturbaciones en la matriz $\Lambda$. En este contexto, el caso particular $N=2$ en un sistema con soporte estructuralmente simétrico permite visualizar de manera directa este mecanismo y proporciona una intuición útil sobre la relación entre la estructura espectral de $\Lambda$ y el umbral de estabilidad. Para interacciones de mutualismo en $\Lambda$ $(--)$, la estabilidad del sistema se encuentra directamente relacionada con la magnitud del ciclo de interacción entre ambas especies. En particular, se obtiene la condición
...
$$
Aplicando el teorema central del límite, cuando $N$ es suficientemente grande, la distribución de la suma \eqref{eqn:Señal-Ruido} se puede aproximar por una variable normal, considerando la varianza obtenida
$$
...
\\
Aunque la relación señal-ruido presenta la misma dependencia funcional en $(p,\sigma,N)$ que el inverso de la escala espectral asociada al criterio clásico de May, ambas cantidades describen propiedades distintas del sistema. Mientras el radio espectral caracteriza la expansión típica del espectro de interacción, el parámetro $\kappa$ cuantifica configuraciones geométricas que favorecen la factibilidad del equilibrio.\\
\\
Esta similitud sugiere una conexión heurística entre estabilidad espectral y factibilidad geométrica, aún cuando ambas cantidades se construyen a partir de configuraciones independientes del espacio de parámetros. Motivados por esta relación de escalas, se introducen los parámetros efectivos
\begin{equation*}
	\begin{split}
		\sigma_c=\frac{1}{\kappa\sqrt{Np}}
	\end{split},\qquad
	\begin{split}
		p_c=\frac{1}{\kappa^2\sigma^2N}
	\end{split}
\end{equation*}
los cuales permiten describir heurísticamente el desplazamiento observado en las transiciones dinámicas del sistema de Lotka--Volterra. Aunque el criterio clásico de May proporciona una escala espectral natural para el inicio de las transiciones, la caracterización precisa del umbral de estabilidad en el modelo generalizado requiere considerar explícitamente la dependencia de la Jacobiana respecto al equilibrio coexistente. Dado que las abundancias $X^*$ introducen un reescalamiento heterogéneo y dependiente de la dinámica, la distribución espectral de $\mathcal{J}$ deja de ser trivial y no puede describirse únicamente mediante argumentos clásicos de matrices aleatorias.
